...
mkdir -p ./log
touch ./log/daizu.log
echo -e "=================================" >> ./log/daizu.log && date | tr '\n' ' ' >> ./log/daizu.log && echo -e "Daizu was started." >> ./log/daizu.log
#
echo -e "\n=================================\nDaizu v1.0.0\n"
...
if [ $SERVICE = 1 ]; then
	SERVICE=MyDNS.JP # This is used to display the service name
	SERVICEID=mydns # This is used in file names
	...
	SERVICE=ieServer # This is used to display the service name
	SERVICEID=ieserver # This is used in file names
	...
	echo -e -n "Type your password.\nPassWord: "
	read PASSWORD
	wget -O - "http://ieserver.net/cgi-bin/dip.cgi?username=$ACCOUNT&domain=$DOMAIN&password=$PASSWORD&updatehost=1" | iconv -f EUC-JP -t UTF-8 | grep post # Succeed when the word "post" is output
	if [ $? = 0 ]; then
